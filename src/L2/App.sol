...
contract App {
    ...
    function gatedHello(address _owner) public {
        ...
        L2_MESSENGER.sendMessage({
            ...
            _message: abi.encodeWithSignature("initiateBridgeUnlock(address,uint32)", _owner, 5_000_000),
    ...
    function batchGatedHello(address[] memory _owners) public {
        ...
        L2_MESSENGER.sendMessage({
            _target: address(L1Hub),
            _message: abi.encodeWithSignature("initiateBatchBridgeUnlock(bytes32,uint32)", _hashedAddresses, 5_000_000),
            _minGasLimit: 5_000_000
        });
        hashedAddresses = 0x0;
    }
